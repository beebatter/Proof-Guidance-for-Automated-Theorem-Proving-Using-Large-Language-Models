...
type lit_or_term =
    LIT of lit
  | TERM of term;;
let string_of_lit_or_term = function
    LIT l -> Fof.string_of_lit l
  | TERM t -> Fof.string_of_term t;;


(*
let string_of_lit_or_term = function
    LIT l ->
      pp_print_string std_formatter "LIT\n"; pp_print_flush std_formatter ();
      let res = Fof.string_of_lit l in
      pp_print_string std_formatter "/LIT\n"; pp_print_flush std_formatter ();
      res
  | TERM t ->
      pp_print_string std_formatter "TERM\n"; pp_print_flush std_formatter ();
      let res = Fof.string_of_term t in
      pp_print_string std_formatter "/TERM\n"; pp_print_flush std_formatter ();
      res;;
*)
  ...
  object(self)
    ...
    method get_var x =
	  ...
	  let node =
		...
		node_src_rev <- (TERM (Cnf.V x)) :: node_src_rev;
		n
    ...
    method get_term = function
	  ...
	  | None ->
	      ...
	      node_src_rev <- (TERM (Cnf.A (f, subterms))) :: node_src_rev;
    ...
    method get_lit (r, subterms) =
	  ...
	  node_src_rev <- (LIT (r, subterms)) :: node_src_rev;
    ...
    method export_node_str () =
      List.map string_of_lit_or_term (List.rev node_src_rev)
