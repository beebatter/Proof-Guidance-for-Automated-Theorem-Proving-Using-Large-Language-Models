...
let load_cnfpremsel fname =
  ...
  (* pp_print_string std_formatter "Ocaml debug\n"; pp_print_flush std_formatter ();*)

  Logic.copend ();
  let axioms_ordered = List.rev (Fof_lexer.file fname)
  and cnf_type (label, ct, cnf) = ct in
  axioms_ordered |> (List.iter (fun (label, ct, cnf) ->
    assert(ct = "plain" || ct = "negated_conjecture" || ct = "extra_terms")));
  let axioms = List.filter (fun (label, ct, cnf) -> ct <> "extra_terms") axioms_ordered
  and extra_terms = List.filter (fun (label, ct, cnf) -> ct = "extra_terms") axioms_ordered in
  let labels = axioms |> List.map (fun (label, ct, cnf) -> if ct = "negated_conjecture" then 1 else 0)
  in
  let remove_header (label, ct, cnf) = cnf in
  let to_cnf c = c |> remove_header |> (unfold_equiv true) |> (dcnf []) in
  let to_clause c = (let [cl] = to_cnf c in cl)
  and process_extra_terms c = List.concat (c |> to_cnf |> List.concat |> (List.map (fun (r, terms) -> terms)) ) in
  List.map to_clause axioms, List.concat (List.map process_extra_terms extra_terms), labels;;


(* This is the premise selection version that expects axiom_redundant and axiom_useful *)
let load_cnfpremsel_real fname =
  ...
  (* pp_print_string std_formatter "END \n"; pp_print_flush std_formatter ();*)
  List.map to_clause axioms, labels;;


let cnfpremsel_cnf_to_indices (clauses, extra_terms, labels) =
  ...
  List.map circ#get_term extra_terms;
  ...
  and node_str = circ#export_node_str ()
  ...
  (
   ...
   [ node_types; symbol_types; clause_types ],
      ...
      *)
   (labels, node_str, symbols)
  );;

let cnfpremsel_cnf_to_indices_real (clauses, labels) =
  let circ = new circuit
  in
  let cl2circ cl =
    circ#empty_sub;
...
let load_cnfpremsel_real_to_ind fname = 
  cnfpremsel_cnf_to_indices_real (load_cnfpremsel_real fname);;
