...
(* parser for iProver - cnfs with negated_conjecture *)
let load_ipremsel fname =
  Logic.copend ();
  let cnfs_ordered = List.rev (Fof_lexer.file fname) in
  cnfs_ordered |> (List.iter (fun (label, ct, cnf) ->
    assert(ct = "plain" || ct = "axiom" || ct = "negated_conjecture" || ct = "axiom_useful" || ct = "axiom_redundant" )));
  (* let axioms = shuffled axioms_ordered in  Shuffle should not affect anything, just to make sure that
     the network cannot exploit a possible bug *)

  (* simplify to one pass!!! *)
  let conjectures = List.filter (fun (label, ct, cnf) -> ct = "negated_conjecture") cnfs_ordered
  (* and conj_pos = cnfs_ordered |> List.map (fun (label, ct, cnf) -> if ct = "negated_conjecture" then 1 else 0) *)
  and axioms = List.filter (fun (label, ct, cnf) -> ct <> "negated_conjecture") cnfs_ordered in
  (* void *)
  let labels = axioms |> List.map (fun (label, ct, cnf) -> if ct = "axiom_useful" then 1 else 0) in
  let to_cnf f = f |> (unfold_equiv true) |> miniscope |> rename_form |> nnf |> skolem |> noforall |> (dcnf []) in
  let remove_header (label, ct, cnf) = cnf in
  let to_cnf_rh_new f = f |> remove_header |> (dcnf []) in
  let to_cnf_rh f = f |> remove_header |> to_cnf in

  (* simply to_cnf - we already have cnfs!!!

     Also modify cl2circ use in premsel_cnf_to_indices!*)
  
  (* List.concat_map to_cnf_rh conjectures, List.map to_cnf_rh axioms, labels;; *)
  List.concat (List.map to_cnf_rh conjectures), List.map to_cnf_rh axioms, labels;;
...
let load_ipremsel_to_ind fname =
  premsel_cnf_to_indices (load_ipremsel fname);;
