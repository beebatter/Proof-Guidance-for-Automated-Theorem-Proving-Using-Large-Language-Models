...
let load_cnfpremsel fname =
  ...
  axioms_ordered |> (List.iter (fun (label, ct, cnf) ->
    assert(ct = "plain" || ct = "negated_conjecture")));
  ...
  in
  (* labels |> (List.iter (fun x -> print_int x));
  print_char '\n';
  Format.print_flush (); *)
...
let cnfpremsel_cnf_to_indices (clauses, clause_types) = 
  ...
  in
  assert ((List.length cl_ind) = (List.length clause_types));
  (
   ...
   [ node_types; symbol_types; clause_types ],
     (*
       ...
       clause_types: 0 = plain, 1 = negated_conjecture
      *)
   symbols
  );;
