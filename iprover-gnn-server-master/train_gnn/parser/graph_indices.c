...
PyObject* py_load_ipremsel(PyObject *self, PyObject *args)
{
  const char* arg;
  if (!PyArg_ParseTuple(args, "s", &arg)) return NULL;
  ...
  PyGILState_STATE gstate = PyGILState_Ensure();
  static value *closure_f = NULL;
  if (closure_f == NULL) closure_f = caml_named_value("cop_load_ipremsel");
  value gi_ml = caml_callback(*closure_f, caml_copy_string(arg));
  long axiom_num;
...
PyObject* py_load_cnfpremsel(PyObject *self, PyObject *args)
{
  ...
  PyObject *node_str = py_caml_str_list(snd(fourth(gi_ml)));
  PyObject *symbols = py_caml_symbols(third(fourth(gi_ml)));
  PyObject *lens_types_symbols = PyTuple_Pack(3, prob_types, node_str, symbols);
  Py_DECREF(prob_types);
  Py_DECREF(symbols);
  Py_DECREF(node_str);
...
PyObject* py_load_cnfpremsel_real(PyObject *self, PyObject *args)
{
  const char* arg;
  if (!PyArg_ParseTuple(args, "s", &arg)) return NULL;
  ...
  if (closure_f == NULL) closure_f = caml_named_value("cop_load_cnfpremsel_real");
