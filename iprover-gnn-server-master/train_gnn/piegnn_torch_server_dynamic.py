...
def load_fnc(fname_pair):
    fname = fname_pair[0]
    fullname = os.path.join(fname_pair[1], fname_pair[0])
    logging.info(f'{fname} {fullname}')
    data, (lens, labels, symbols) = cop.load_premsel(fullname)
    return (GraphData(data), (lens, labels)), fname
...
def load_data(datadir, sample=-1):
    ...
    logging.info(f"Number of files to load: {len(fnames)}")
    test = False
    if not test:
    ...
    for fname in fnames:
        logging.info(f'{fname} {os.path.join(datadir, fname)}')
...
def dynamic_load_data(datadirs, use_non_dynamic, max_static_examples, samples_per_problem, args, sample=-1):
    logging.info(f'The number of input dirs: {len(datadirs)}')

    use_static_examples = min(len(datadirs), max_static_examples)

    if use_non_dynamic:
        logging.info(f'The number of used samples per problem: {use_static_examples}')
    else:
        logging.info(f'The number of dynamic samples per problem: {samples_per_problem}')
    
    fnames_lists = []
    
    for datadir in datadirs:
        
        logging.info(f'Listing the input dir {datadir}')
        fnames_lists.append(os.listdir(datadir))

    fnames_non_dynamic_dict = {}
        
    for i, fnames_list in enumerate(fnames_lists):
        for fname in fnames_list:
            if '.' not in fname:
                if fname not in fnames_non_dynamic_dict:
                    fnames_non_dynamic_dict[fname] = {(fname, i)}
                else:
                    fnames_non_dynamic_dict[fname].add((fname, i))

    if not use_non_dynamic:
        fnames_dynamic_dict = {}                    
        ...
        for i, fnames_list in enumerate(fnames_lists):
            for fname in fnames_list:
                if '.' in fname:
                    ...
                    if (fname_prefix in fnames_non_dynamic_dict) and ((fname_prefix, i) in fnames_non_dynamic_dict[fname_prefix]):
                        if fname_prefix not in fnames_dynamic_dict:
                            fnames_dynamic_dict[fname_prefix] = {(fname, i)}
                        else:
                            fnames_dynamic_dict[fname_prefix].add((fname, i))
                    else:
                        logging.info(f'{i, fname} has no classical file (without .); no given clauses, but scores_req and hence an example. IT IS NOT USED!!!')
                        
    if use_non_dynamic:
        fnames = list(fnames_non_dynamic_dict.keys())
        ...
        for fname in fnames:
            num_examples = len(fnames_non_dynamic_dict[fname])
            
            if num_examples > use_static_examples:
                fnames_pairs.extend(random.sample(list(fnames_non_dynamic_dict[fname]), k=use_static_examples))
            else:
                fnames_pairs.extend(list(fnames_non_dynamic_dict[fname]))
                fnames_pairs.extend(random.choices(list(fnames_non_dynamic_dict[fname]), k=use_static_examples - num_examples))

    else:
        fnames = list(fnames_dynamic_dict.keys())
        
        if not sample == -1:
            if sample > len(fnames):
                sample = len(fnames)
            used_problems = random.sample(fnames, sample)
        ...
        for fname in fnames:
            if not args.use_zero_dynamic:
                num_examples = len(fnames_dynamic_dict[fname])

                if num_examples > samples_per_problem:
                    fnames_pairs.extend(random.sample(list(fnames_dynamic_dict[fname]), k=samples_per_problem))
                else:
                    fnames_pairs.extend(list(fnames_dynamic_dict[fname]))
                    fnames_pairs.extend(random.choices(list(fnames_dynamic_dict[fname]), k=samples_per_problem - num_examples))
                
                #fnames_pairs.extend(random.choices(list(fnames_dynamic_dict[fname]), k=samples_per_problem))
            else:
                zero_pair = []
                for fname_pair in fnames_dynamic_dict[fname]:
                    # Find some 0 iteration example, they should be all the same...?!
                    if fname_pair[0].endswith('.0'):
                        zero_pair.append(fname_pair)
                        break

                assert len(zero_pair) == 1, [zero_pair, fname, fnames_dynamic_dict[fname]]

                fnames_pairs.append(zero_pair[0])

                num_examples = len(fnames_dynamic_dict[fname])

                if num_examples > samples_per_problem - 1:
                    fnames_pairs.extend(random.sample(list(fnames_dynamic_dict[fname]), k=samples_per_problem - 1))
                else:
                    fnames_pairs.extend(list(fnames_dynamic_dict[fname]))
                    fnames_pairs.extend(random.choices(list(fnames_dynamic_dict[fname]), k=samples_per_problem - (num_examples + 1)))


                
                #fnames_pairs.extend(random.choices(list(fnames_dynamic_dict[fname]), k=samples_per_problem - 1))

    random.shuffle(fnames_pairs)
            
    logging.info(f"Number of files to load: {len(fnames_pairs)} (more samples per problem possible)")

    data_list = []
    fnames_out = []

    with concurrent.futures.ProcessPoolExecutor(max_workers=16) as pool:
        map_result = pool.map(load_fnc, [(x, datadirs[y]) for x, y in fnames_pairs], chunksize=10)

        for d, f in map_result:
            data_list.append((d, f))
            fnames_out.append(f)

    return data_list, fnames_out
    ...
    def graph_conv(self, node_vectors, graph_structure, layer):
        ...
        for e, n in enumerate(graph_structure.node_inputs):
            ...
            if not len(n.nodes) == 0:
                ...
                #    logging.error(e)
                #    logging.error(torch.tensor(n.nodes))
                #    raise
                # print("gathered")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_non_dynamic',
                        action='store_true',
                        help='use non dynamic examples (not containing .)')

    parser.add_argument('--use_zero_dynamic',
                        action='store_true',
                        help='use zero dynamic')
    
    parser.add_argument('--samples_per_problem',
                        type=int,
                        default=10,
                        help='the number of dynamic samples per problem')
    ...
    parser.add_argument('--max_static_examples',
                        type=int,
                        default=6,
                        help='the maximal number of static examples')
    ...
    parser.add_argument("--no_splits",
                        action="store_true",
                        help='dot not use train/test splits')

    parser.add_argument('--train_list',
                        type=str,
                        default='../experiments/split_train.txt',
                        help='train problems list file')

    parser.add_argument('--test_list',
                        type=str,
                        default='../experiments/split_test.txt',
                        help='test problems list file')
    ...
    logging.basicConfig(level=logging.INFO,
                        ...
                        handlers=[
                            logging.FileHandler(os.path.join(args.output_data_dir, "output.log")),
    ...
    torch.save(model.state_dict(), os.path.join(args.output_data_dir, "pt_server_test_model.pt"))
    ...
    total_data_pair, fnames_out = dynamic_load_data(
        args.input_data_dir, args.use_non_dynamic, args.max_static_examples, args.samples_per_problem, args, sample=-1
    )  # -1 means take all
    batch_size = 4
    j = 0
    #vb = total_data[j: j + batch_size]
    vb = [x[0] for x in total_data_pair[j: j + batch_size]]

    float_predictions = model.predict(
        vb
    )

    logging.info(float_predictions)
    ...
    if not args.no_splits:
        ...
        with open(args.train_list) as f:
            for l in f:
                train_split.add(l.strip())

        test_split = set()
        with open(args.test_list) as f:
            for l in f:
                test_split.add(l.strip())

        for d, f in zip(total_data_pair, fnames_out):
            assert d[1] == f, [d[1], f]
            fname = f.split('.')[0]
            if fname in train_split:
                train_data.append(d[0])
                logging.info(f'Traindata: {fname}')
            elif fname in test_split:
                val_data.append(d[0])
                logging.info(f'Validdata: {fname}')
            else:
                logging.info(f'Ignoreddata: {fname}')
    else:
        logging.info('Do not use splits!!!')
        no_samples = len(total_data)
        train_end = int(np.floor(no_samples * 0.95))

        random.shuffle(total_data)

        #train_data = total_data[:train_end]    
        #val_data = total_data[train_end:]
        train_data = [x[0] for x in total_data[:train_end]]
        val_data = [x[0] for x in total_data[train_end:]]
    ...
    logging.info(f'Valid data: {len(val_data)}')
    ...
    for i in range(num_epochs):
        ...
        for j in range(0, len(train_data), batch_size):
            ...
            b = train_data[j : j + batch_size]
            graph_info, labels = batch_graphs(b)
            try:
                premsel_cur = model.train((graph_info, labels), optimizer, loss_function)
                update_accum(premsel_accum, premsel_cur)
                epoch_loss += premsel_cur[0]
                losses.append(premsel_cur[0])
            except:
        ...
        for j in range(0, len(val_data), batch_size):
            ...
            vgraph_info, vlabels = batch_graphs(vb)
            try:
                premsel_cur, premsel_num = model.evaluation(
                    (vgraph_info, vlabels), loss_function
                )
        ...
        logging.info("Evaluation {}: Premsel {}".format(i, stats_str(premsel_stats),))
        torch.save(
            model.state_dict(), os.path.join(args.output_data_dir, f"model_{expname}_epoch_{i}.pt")
        )
    logging.info(losses)
    #plt.plot(losses)
    #plt.show()
