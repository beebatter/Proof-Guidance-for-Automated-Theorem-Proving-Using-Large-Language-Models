...
//DEBUG fprintf(stderr,"Exit recursion child at position %d\n",ChildIndex);
	    ...
	    //                fprintf(stderr,"YIKES, recursion depth 100 seems crazy\n");
                fprintf(stderr,"YIKES, recursion depth 1000 seems crazy\n");
            }
        }
    }
...
//----children, so the tree is broken.
...
//----If global interrupted, then send it on. If a SIGALRM, convert to SIGXCPU
//----because some bastards ignore SIGALRM, and some time gets lost somehow.
    if (NumberInTree > 0 && GlobalInterrupted) {
        if (GlobalSignalReceived == SIGALRM) {
            GlobalSignalReceived = SIGXCPU;
        }
...
//            PrintTimes("FINAL WATCH",TreeCPUTime,WCTime);
            ...
            PrintTimes("USED TIME",TreeCPUTime,WCTime);
