...
let incr_clause_counter () = incr clause_global_counter
...
let bc_ac_axiom = 12
let bc_ext_axiom = 13 (* is external theory axiom e.g. supplied by Vampire during clausification *)
let bc_has_interpreted_symbols = 14
...
type basic_clause = {
  ...
  mutable length : int;	(* number lits *)
  mutable num_of_symb : int;  (* number of all symbols in literals *)
  mutable num_of_var : int;  (* number of all vars in literals *)
...
type clause = {
  ...
  (* mutable context_id : int; *)  (* clause is identified by context_id and fast_id *)
    ...
    mutable tstp_source : tstp_source param; (* can be changed in reconstruction of global subsumption and prop_imp *)
    ...
    (* conjecture distance *)
    mutable conj_dist : int;

    mutable when_born : int param; (* TODO change to local *)
    mutable abstr_ref_concr : bool param; (* TODO change to local *)
    mutable cl_rnd : int; (* a fixed random number associated with a clause, used in e.g. rnd queues *)
   }
...
and tstp_inference_rule =
  ...
  | Global_subsumption of int (* max clause id for justification *)
  | Global_subsumption_just 
  | Prop_impl of int
  | Prop_impl_just 
  | SMT_impl_just
  | Bin_hyper_res 
  ...
  | Equality_resolution
  | Equality_factoring
  | Predicate_to_equality
  | Predicate_to_equality_rev
  | WellDefinedness
  | Demodulation
  | LightNormalisation
  | EqFlip
...
module BClause_Weak_Key =
  struct
    ...
    let equal  bc1 bc2 =
      try
        List.for_all2 (==) (bc_get_lits bc1) (bc_get_lits bc2)  (* andrepd: More efficient if lazy *)
      with
        Invalid_argument _ -> false
...
let get_bc_fast_key c = c.basic_clause.bc_fast_key
...
let bc_compare bc1 bc2 = Int.compare (bc_get_fast_key bc1) (bc_get_fast_key bc2)
...
let nequal_bc c1 c2 = c1.basic_clause != c2.basic_clause
let hash_bc c =  (bc_get_fast_key (get_bc c))

(* Operators *)
module Bc = struct
  let equal = equal_bc
  let nequal = nequal_bc
  let (==) = equal_bc
  let (!=) = nequal_bc
  let hash = hash_bc
...
let compare_clause c1 c2 =
  Int.compare (get_fast_key c1) (get_fast_key c2)
...
let max_atom_input_occur_lits lits =
  let get_symb lit =
    ...
    | _ -> failwith (sprintf "assign_max_atom_input_occur should not be var: lits: %s" (Term.term_list_to_string lits))
  in
  let cmp lit1 lit2 =
...
let min_defined_symb_lits lits =
  let cmp_lit l1 l2 =
    ...
    match d1, d2 with
    | Def(i1), Def(i2) -> Int.compare i1 i2
...
let create_basic_clause_non_weak lits =
  ...
  with Not_found -> 
    ...
    if List.X.is_nonempty lits then fill_bc_auto_params bc;
...
let create_basic_clause_weak lits =
  ...
  with Not_found ->
    (* let bc = bc_template ~bc_fast_key: (!bc_global_counter) lits in *)
    bc_global_counter := !bc_global_counter + 1;
    if List.X.is_nonempty lits then fill_bc_auto_params bc_template;
    BC_WHtbl.add bc_clause_db_weak bc_template;
    bc_template

let incr_gc_bc_elim _c = 
  Statistics.(bump_int_stat gc_basic_clause_elim)
...
let create_basic_clause lits = 
  if !global_options.clause_weak_htbl then 
    let new_clause = create_basic_clause_weak lits in 
    if !global_options.gc_record_bc_elim then Gc.finalise incr_gc_bc_elim new_clause;
    new_clause
  else
...
let out_tptp = to_stream_tptp stdout_stream

let to_string =
  to_string_fun_from_to_stream_fun 100 to_stream

let to_string_tptp =
  to_string_fun_from_to_stream_fun 100 to_stream_tptp
...
let is_ac_axiom c =
  bc_get_bool_param bc_ac_axiom c

let is_ext_axiom c = 
  bc_get_bool_param bc_ext_axiom c

let assign_is_eq_axiom value c = (* user *) (* assign only for user parameters *)
  bc_set_bool_param value bc_eq_axiom c

let assign_is_ac_axiom value c = 
  bc_set_bool_param value bc_ac_axiom c

let assign_is_ext_axiom value c = 
  bc_set_bool_param value bc_ext_axiom c

let has_interpreted_symbols c =
  bc_get_bool_param bc_has_interpreted_symbols c

let assign_has_interpreted_symbols value c = 
  bc_set_bool_param value bc_has_interpreted_symbols c

let is_unit c =
  c.basic_clause.length = 1

let is_unit_eq c =
  match c.basic_clause.lits with
  | [x] when Term.Eq.is_eq x -> true
  | _ -> false

let is_pos_unit_eq c =
  match c.basic_clause.lits with
  | [x] when Term.Eq.is_pos_eq x -> true
  | _ -> false
...
(* Used several times, so I've put it here *)
(* No completeness check needed at all if: non-unit, negative literal, predicate literal. *)
let demod_no_check clause = 
  match get_lits clause with
  | [x] ->
    begin match Term.Eq.decompose_atom x with
    | Some (l,r) -> 
      (* r == SystemDBs.top_term *) 
      (* Term.get_top_symb r == Symbol.symb_top *)
      (match r with Term.Fun(sym,_,_) when sym == Symbol.symb_top -> true | _ -> false)
    | None -> true
    end
  | _ -> true
...
(* only should be used in reconstruction of global subusmption and prop_imp. *)
let reassign_tstp_source tstp_source c =
...
let is_simplification_inf tstp_source = 
  match tstp_source with
  | TSTP_inference_record
      (tstp_inference_rule, _main_parents) ->        
        begin
	  match tstp_inference_rule with
          | Global_subsumption _   (* important for performace *)
          | Global_subsumption_just   (* should it be included ? only occurs in proof reconstruction *)
          | Forward_subsumption_resolution
          | Backward_subsumption_resolution
          | Eq_res_simp
          | Dis_eq_elim
...
let assign_ps_when_born i c =
  ...
  |Def _ -> ()  (* can not reassign when born as this can mess up priority queues! *)
                (* TODO make when born local *)
                (* failwith ("assign_ps_when_born: already def:"^(to_string c)) *)
  |Undef -> c.when_born <- Def(i)
...
let when_born_concl prem1 prem2 clause =
  let born_list1 : int list = List.map get_ps_when_born prem1 in
  let born_list2 : int list = List.map get_ps_when_born prem2 in
  let inv_compare = Ord.reverse_f Int.compare in
  (* finds min element *)
  ...
  let max_born = max min_prem1 min_prem2 in  
...
(* negative numbers interpreted as infinite conj distance; 0 neg_conjecture; positive is the proof distance *)

type conj_dist = int (* declare as abstract type as operations are different from int *)
      
let get_conj_dist c =
  c.conj_dist

let assign_conj_dist d c =
  c.conj_dist <- d

let inf_conjecture_dist = -1
let conjecture_int = 0

let is_inf_conj_dist d = (d = inf_conjecture_dist)

let conj_dist_to_string = string_of_int

let conj_dist_to_int d = d
  
let is_negated_conjecture c =
  c.conj_dist = conjecture_int
(*  bc_get_bool_param bc_is_negated_conjecture c *)

let cmp_conj_dist_num d1 d2 =
  if (d1>= 0 && d2 >= 0) then Int.compare d1 d2
  else if (d1 < 0 && d2 >= 0) then Ord.gt (* negative greater dist.*)
  else if (d1 >= 0 && d2 < 0) then Ord.lt
  else if (d1 = d2) then Ord.eq
  else (assert (d1 <0  && d2 < 0); Ord.eq)
    
let cmp_conj_dist c1 c2 =
  Ord.lift get_conj_dist cmp_conj_dist_num c1 c2
    
let inc_conj_dist d =
  if d < 0 then d else d + 1

let get_min_conj_dist_cl c_list =
  assert (List.X.is_nonempty c_list);
  List.X.min cmp_conj_dist c_list 
  
(* if empty list then inf_distance *)  
let get_min_conj_dist c_list =
  if (List.X.is_empty c_list) then inf_conjecture_dist
  else
    let min_c = get_min_conj_dist_cl c_list in
    get_conj_dist min_c

  
let get_conj_dist_tstp_source tstp_source =
  let parents = get_parents_tstp tstp_source in
  get_min_conj_dist parents

let inherit_conjecture from_c to_c = 
  to_c.conj_dist <- (min_cmp cmp_conj_dist_num from_c.conj_dist to_c.conj_dist)

let split_neg_conjectures cls = 
  List.partition is_negated_conjecture cls  
  

(* get all clauses with the smallest conj distance *)
(* if all clauses have max conj distance then return ([],cls) *)

let split_smallest_dist_neg_conj cls =
  if (List.X.is_empty cls) then ([],cls)
  else
    let min_conj_dist = get_min_conj_dist cls in 
    
    if (is_inf_conj_dist min_conj_dist)
    then
      ([],cls)   
    else
      List.partition (fun c -> (get_conj_dist c) = min_conj_dist) cls
        


      

(*-------- Abstraction-Refinment concrete --------------*)
let assign_ps_abstr_ref_concr v c =
  c.abstr_ref_concr <- Def(v)

let get_ps_abstr_ref_concr c =
  match c.abstr_ref_concr with
  | Def(v) -> v
  | Undef -> true
...
let rec pp_clause_list_tptp ppf = function
  ...
  | c :: tl when Poly.(get_tstp_source c = TSTP_external_source (TSTP_theory TSTP_equality)) ->
...
let apply_bsubst term_db_ref bsubst bound lits =
  let bterm_list = Bind.bind_list bound lits in
  let new_lit_list =
...
let cmp_arity s1 s2 = 
  try
    Int.compare (Symbol.get_arity s1) (Symbol.get_arity s2) 
  with 
    Symbol.Arity_undef -> 0 

let cmp_symb = lex_combination2 cmp_arity Symbol.compare
...
(* let rec term_compare' t s =
  ...
  | (Term.Fun(t_sym, t_args, _), Term.Fun(s_sym, s_args, _)) ->
      ...
      then 
        ...
        if cmp > cequal 
        then raise_notrace Term_compare_greater
        else raise_notrace Term_compare_less
  | (Term.Var(_, _), Term.Fun(_, _, _)) -> raise_notrace Term_compare_greater
  | (Term.Fun(_, _, _), Term.Var(_, _)) -> raise_notrace Term_compare_less
...
let rec term_compare' t s =
  ...
  | Term.Fun(t_sym, t_args, _), Term.Fun(s_sym, s_args, _) ->
    let cmp = cmp_symb t_sym s_sym in
    if cmp = cequal then 
      let t_args = Term.arg_to_list t_args in 
      let s_args = Term.arg_to_list s_args in 
      List.X.compare_lex term_compare' t_args s_args  (* never different length or compare_arity would not be eq *)
    else
      cmp
  | Term.Var _, Term.Fun _ -> cgreater
  | Term.Fun _, Term.Var _ -> cless
  | Term.Var _, Term.Var _ -> cequal
...
let norm_bterm_wrt_subst bound_t bound_subst =
  match bound_t with
  | (b_t, Term.Var(t_v, _)) -> (
     try SubstBound.find_vnorm (b_t, t_v) bound_subst
...
let bterm_subst_compare bound_t bound_s bound_subst =
  ...
  let pre_comp_fun =
    if !global_options.bmc1_incremental
    then
      cmp_bmc1_atom_fun ()
    else
  ...
  let cmp_res = 
    ...
    then    
      (
       ...
       if cmp_b_atoms = 0 
       then (* compare signs *)
         - ( Term.cmp_sign (Bind.unbind bound_t) (Bind.unbind bound_s)) (* neg larger *)
       else
         cmp_b_atoms
      )      
    else
...
(* Hotspot in profiling, this is as much as 3x faster than before *)
let rec bterm_subst_compare' bound_t bound_s bound_subst =
  let norm_bt = norm_bterm_wrt_subst bound_t bound_subst in
  let norm_bs = norm_bterm_wrt_subst bound_s bound_subst in
  let (b_t, t) = norm_bt in
  let (b_s, s) = norm_bs in
  match t, s with
  | Term.Fun(t_sym, t_args, _), Term.Fun(s_sym, s_args, _) ->
    let cmp = cmp_symb t_sym s_sym in
    dbg D_norm @@ lazy (sprintf "cmp_symb: t_sym: %s s_sym: %s cmp: %s"
      (Symbol.to_string t_sym) (Symbol.to_string s_sym) (string_of_int cmp)
    );
    if cmp = Ord.eq then
      let t_args = Term.arg_to_list t_args in
      let s_args = Term.arg_to_list s_args in
      List.X.compare_lex (fun t' s' -> bterm_subst_compare' (b_t, t') (b_s, s') bound_subst) t_args s_args
                    (* (fun _ t' s' -> bterm_subst_compare' (b_t, t') (b_s, s') bound_subst) *)
    else
      cmp
  | _ -> 
    term_compare' t s
...
let bterm_subst_compare bound_t bound_s bound_subst =
  ...
  let (bt, atom_t) = b_atom_t in
  let (bs, atom_s) = b_atom_s in
  (* let (bt, lit_t) = bound_t in
  let (bs, lit_s) = bound_s in
  let atom_t = Term.get_atom lit_t in
  let atom_s = Term.get_atom lit_s in *)
  ...
  let pre_comp_fun =
    if !global_options.bmc1_incremental then
      ...
      Term.lit_cmp_type_to_fun (Lit_eq false)
      (* Ord.reverse_f cmp_is_eq_lit *)
  in
  let pre_comp = pre_comp_fun atom_t atom_s in

  let cmp_res = 
    if pre_comp = Ord.eq then (
     let cmp_b_atoms = 
       bterm_subst_compare' b_atom_t b_atom_s bound_subst
     in 
     if cmp_b_atoms = Ord.eq then (* compare signs *)
       -(Term.cmp_sign (Bind.unbind bound_t) (Bind.unbind bound_s)) (* neg larger *)
     else
       cmp_b_atoms
    ) else (
...
let normalise_lit_list ?(normalise_eqs=false) term_db_ref lit_list =
  (* in some cases lits are not yet added to term_db *)
  let db_lit_list = 
    if not normalise_eqs then
      List.map (fun term -> TermDB.add_ref term term_db_ref) lit_list 
    else
      (* List.map (fun term -> TermDB.normalise_eq_lit term term_db_ref) lit_list  *) undefined()
  in
  ...
  let norm_lit_list = normalise_blitlist_list term_db_ref (SubstBound.create ()) [(1, removed_duplicates)] in
  dbg D_create @@ lazy (sprintf "before norm: %s" (Term.term_list_to_string removed_duplicates));
  dbg D_create @@ lazy (sprintf "after  norm: %s\n" (Term.term_list_to_string norm_lit_list));
  norm_lit_list
...
let new_clause ~is_negated_conjecture ?(bc_imp_inh=bc_imp_inh_default_val) tstp_source bc =
  let conj_dist =
    if is_negated_conjecture then 
      ...
      inc_conj_dist (get_conj_dist_tstp_source tstp_source)

(* TODO: investigate: For some reason not increasing conj distance if the inf is simplification does not work as well  *)
(*
      let srouce_distance = get_conj_dist_tstp_source tstp_source in      
      if (is_simplification_inf tstp_source) then
        min srouce_distance max_conjecture_dist
      else 
        min (srouce_distance + 1) max_conjecture_dist
*)
  in
  let bc_imp_inh_final = 
    ...
    list_find_min_element Int.compare (bc_imp_inh::imp_parent_list)
  in
...
(*  bc_set_bool_param is_negated_conjecture bc_is_negated_conjecture new_clause; *)
  ...
  let parents = get_parents_tstp tstp_source in
  (* don't bump for simplifying inferences; new 2020/12 *)
  if is_simplification_inf tstp_source then (
    let born_list : int list = List.map get_ps_when_born parents in
    let max_born = list_find_max_element_zero Int.compare born_list in
    assign_ps_when_born max_born new_clause; 
  ) else (
...
(* TODO: conj_dist from the rest of the code to here*)
 *)

let create_clause_opts ~is_negated_conjecture ?(bc_imp_inh=bc_imp_inh_default_val) ?(normalise_eqs=false) term_db_ref tstp_source lits =
  let lits' = normalise_lit_list ~normalise_eqs term_db_ref lits in
...
let create_clause term_db_ref ?(is_negated_conjecture=false) ?(bc_imp_inh=bc_imp_inh_default_val) ?(normalise_eqs=false) tstp_source lits =
  create_clause_opts ~is_negated_conjecture ~bc_imp_inh ~normalise_eqs term_db_ref tstp_source lits

(* assume clause is a negate_conjecture *)
(* only used in parsing and a couple other places *)
...
(*-------------- New clause creation and normalisation -------------------*)

module NewCreateClause = struct
  (* Total ordering on terms *)

  (* TODO: Properly implement this in a complete way, see notes *)
  let preorder_term a b =
    let cmp1 = Int.compare (Term.get_num_of_symb b) (Term.get_num_of_symb a) in
    if cmp1 <> Ord.eq then cmp1 else
    let cmp2 = Int.compare (Term.get_num_of_var b) (Term.get_num_of_var a) in
    cmp2

  let rec total_ordering_term a b =
    let cmp1 = preorder_term a b in
    if cmp1 <> Ord.eq then cmp1 else
    match a, b with
    | Term.Fun (sym_a, args_a, _), Term.Fun (sym_b, args_b, _) ->
      let cmp_symb = Symbol.compare_fast_key sym_a sym_b in
      if cmp_symb <> Ord.eq then cmp_symb else
      List.X.compare ~cmp:total_ordering_term (Term.arg_to_list args_a) (Term.arg_to_list args_b)
    | Term.Fun _, Term.Var _ -> 
      Ord.gt
    | Term.Var _, Term.Fun _ -> 
      Ord.lt
    | Term.Var (v_a, _), Term.Var (v_b, _) -> 
      (* Vars have to be the same type, so just compare by val *)
      (* dassert (fun () -> Var.get_type v_a == Var.get_type v_b);
      Ord.lift Var.get_var_val Int.compare v_a v_b *)
      (* Actually that only holds outside top-level of equalities... *)
      Var.compare v_a v_b

  let total_ordering_atom a b =
    match Term.Eq.decompose_atom a, Term.Eq.decompose_atom b with
    | Some (al, ar), Some (bl, br) ->
      (* At this stage assume we have already ordered equations such that al>ar and bl>br *)
      (* Well actually not *)
      let cmp1 = total_ordering_term al bl in
      if cmp1 <> Ord.eq then cmp1 else
      let cmp2 = total_ordering_term ar br in
      cmp2
    | None, Some _ -> Ord.gt
    | Some _, None -> Ord.lt
    | None, None -> total_ordering_term a b

  let total_ordering_lit a b = 
    let sign1, atom1 = Term.split_sign_lit a in
    let sign2, atom2 = Term.split_sign_lit b in
    (* Negatives go first *)
    let cmp_sign = Bool.compare sign1 sign2 in
    if cmp_sign <> Ord.eq then cmp_sign else
    total_ordering_atom atom1 atom2
  ...
  module Renaming = struct
    type t = {
      ...
      mutable map: term (* var *) VMap.t;
      (* Next unbound variable of a given type *)
      mutable next_free: int SMap.t;
    }

    let empty() = {
      map = VMap.empty;
      next_free = SMap.empty;
    }

    let copy x = {
      map = x.map;
      next_free = x.next_free;
    }

    let move x y = 
      x.map <- y.map;
      x.next_free <- y.next_free

    let add x y env = 
      env.map <- env.map |> VMap.add x y

    let find_opt x env = 
      env.map |> VMap.find_opt x

    (* Get the fresh var of that type, and also record tha  *)
    let new_var term_db_ref typ env = 
      let n = ref 0 in
      let next_free' = 
        env.next_free |> SMap.update typ (function
          | None -> Some 1
          | Some x -> n := x; Some (x+1)
        )
      in
      (* let env' = {env with next_free = next_free'} in
      env', n *)
      env.next_free <- next_free';
      (* !n *)
      let t = Term.create_var_term (Var.create typ !n) in
      TermDB.add_ref t term_db_ref

    let rec normalise_term term_db_ref env t = 
      if ((* try *) Term.is_ground t (* with Term.Num_of_var_undef -> false *)) then t else
      match t with
      | Term.Fun (sym, args, _) -> 
        (* Filter out, from context, literals which cannot be compatible with term *)
        let args' = 
          args |> Term.arg_map_left (fun t' ->
            normalise_term term_db_ref env t'
          )
        in
        if List.X.equal ~eq:(==) (Term.arg_to_list args) (Term.arg_to_list args') then
          t
        else
          Term.create_fun_term_args sym args'
      | Term.Var (var, _) -> 
        begin match find_opt var env with
        | Some var' -> var'
        | None -> 
          let var' = env |> new_var term_db_ref (Var.get_type var) in
          env |> add var var';
          var'
        end

    let normalise_atom term_db_ref env atom = 
      normalise_term term_db_ref env atom

    let normalise_lit normalise_eqs term_db_ref env lit =
      if not normalise_eqs then
        normalise_atom term_db_ref env lit
      else
        let sign, atom = Term.split_sign_lit lit in
        match Term.Eq.decompose_atom_type atom with
        | None -> normalise_atom term_db_ref env lit
        | Some (typ,l,r) -> 
          (* If both are ground then just compare and return *)
          (* if Term.is_ground l && Term.is_ground r then *)

          (* If can be pre-ordered, there's only one orientation to try *)
          let preorder = preorder_term l r in
          if preorder = Ord.lt then
            normalise_atom term_db_ref env lit
          else if preorder = Ord.gt then
            (* let atom' = Term.create_fun_term (Symbol.symb_typed_equality) [typ;r;l] in
            let lit' = if sign then atom' else Term.create_neg_lit atom' in
            normalise_atom term_db_ref env lit' *)
            let r' = normalise_term term_db_ref env r in
            let l' = normalise_term term_db_ref env l in
            let atom' = Term.create_fun_term (Symbol.symb_typed_equality) [typ;r';l'] in
            let lit' = if sign then atom' else Term.create_neg_lit atom' in
            lit'
          else (
            (* Save the env copy *)
            let env1 = env in
            let env2 = copy env in
            (* Try forwards *)
            (* let atom1 = 
              atom
              |> normalise_atom term_db_ref env1
            in
            let atom2 = 
              Term.create_fun_term (Symbol.symb_typed_equality) [typ;r;l]
              |> normalise_atom term_db_ref env2 
            in *)
            (* With env1: first l then r *)
            let l1 = normalise_term term_db_ref env1 l in
            let r1 = normalise_term term_db_ref env1 r in
            (* With env2: first r then l *)
            let r2 = normalise_term term_db_ref env2 r in
            let l2 = normalise_term term_db_ref env2 l in
            (* Add to term_db *)
            let l1 = TermDB.add_ref l1 term_db_ref in
            let r1 = TermDB.add_ref r1 term_db_ref in
            let l2 = TermDB.add_ref l2 term_db_ref in
            let r2 = TermDB.add_ref r2 term_db_ref in
            (* Orient *)
            let l1,r1 = if total_ordering_term l1 r1 <= Ord.eq then l1,r1 else r1,l1 in
            let l2,r2 = if total_ordering_term l2 r2 <= Ord.eq then l2,r2 else r2,l2 in
            (* Finally, compare and build the preferred one *)
            let cmp = 
              let cmp1 = total_ordering_term l1 l2 in
              if cmp1 <> Ord.eq then cmp1 else
              let cmp2 = total_ordering_term r1 r2 in
              cmp2
            in
            (* l1,r1 wins *)
            if cmp <= Ord.eq then (
              if l1 == l && r1 == r then lit else
              let atom' = Term.create_fun_term (Symbol.symb_typed_equality) [typ;l1;r1] in
              let lit' = if sign then atom' else Term.create_neg_lit atom' in
              lit'
            )
            (* l2,r2 wins *)
            else (* if cmp = Ord.gt then *) (
              move env env2;  (* Don't forget to proceed with the env2 *)
              let atom' = Term.create_fun_term (Symbol.symb_typed_equality) [typ;l2;r2] in
              let lit' = if sign then atom' else Term.create_neg_lit atom' in
              lit'
            )
          )
  ...
  let normalise_lits term_db_ref ~normalise_eqs lits = 
    dbg D_norm @@ lazy (sprintf "normalise_lits: %s" (List.X.to_string ~first:"( " ~last:" )" ~sep: " | " Term.to_string lits));
    (* Normalise eq literals *)
    (* let lits' = 
      if normalise_eqs then
        lits |> List.map (fun lit -> 
          let sign, atom = Term.split_sign_lit lit in
          match Term.Eq.decompose_atom_type atom with
          | None -> lit
          | Some (typ,l,r) -> 
            if total_ordering_term l r <= Ord.eq then
              lit
            else
              let atom' = Term.create_fun_term (Symbol.symb_typed_equality) [typ;r;l] in
              if sign then atom' else Term.create_neg_lit atom'
        )
      else
        lits
    in *)
    (* Add term db *)
    let lits = List.map (fun x -> TermDB.add_ref x term_db_ref) lits in
    (* Sort *)
    let lits' = List.X.sort_uniq total_ordering_lit lits in
    dbg D_norm @@ lazy (sprintf "sorted:         %s" (List.X.to_string ~first:"( " ~last:" )" ~sep: " | " Term.to_string lits'));
    (* Rename *)
    let env = Renaming.empty () in
    let lits'' = 
      lits' |> List.map (fun x ->
        let x' = Renaming.normalise_lit normalise_eqs term_db_ref env x in
        if x' == x then x' else TermDB.add_ref x' term_db_ref
      )
    in
    dbg D_norm @@ lazy (sprintf "renamed:        %s" (List.X.to_string ~first:"( " ~last:" )" ~sep: " | " Term.to_string lits''));
    (* dbg_env D_norm (fun () ->
      if not (List.X.equal ~eq:(==) lits' lits'') then (
        dbg D_norm @@ lazy (sprintf "sorted: %s" (List.X.to_string ~first:"( " ~last:" )" ~sep: " | " Term.to_string lits'));
        dbg D_norm @@ lazy (sprintf "renamed:%s" (List.X.to_string ~first:"( " ~last:" )" ~sep: " | " Term.to_string lits''));
      )
    ); *)
    (* After rename, we wouldn't gain anything by sorting again *)
    (* dassert (fun () -> List.X.equal ~eq:(==) lits'' (List.sort_uniq total_ordering_lit)); *)
    (* But this is obviously not true *)
    let lits''' = 
      if List.X.equal ~eq:(==) lits'' lits' then 
        lits'' 
      else 
        List.X.sort_uniq total_ordering_lit lits'' 
    in
    dbg D_norm @@ lazy (sprintf "resorted:       %s" (List.X.to_string ~first:"( " ~last:" )" ~sep: " | " Term.to_string lits'));
    lits'''

  let create_clause 
      term_db_ref 
      ?(is_negated_conjecture=false) 
      ?(bc_imp_inh=bc_imp_inh_default_val) 
      ?(normalise_eqs=false) 
      tstp_source lits =
    let lits' = normalise_lits term_db_ref ~normalise_eqs lits in
    let bc = create_basic_clause lits' in
    new_clause ~is_negated_conjecture ~bc_imp_inh tstp_source bc

  let create_neg_conjecture term_db_ref ?(bc_imp_inh=bc_imp_inh_default_val) tstp_source lits =
    create_clause ~is_negated_conjecture:true ~bc_imp_inh ~normalise_eqs:true term_db_ref tstp_source lits

  let create_clause_raw = create_clause_raw
end

let normalise_term_list term_db_ref tlist = 
  let env = NewCreateClause.Renaming.empty () in
  List.map (NewCreateClause.Renaming.normalise_term term_db_ref env) tlist

let create_clause = NewCreateClause.create_clause
let create_neg_conjecture = NewCreateClause.create_neg_conjecture
let create_clause_raw = NewCreateClause.create_clause_raw
...
let cmp_num_var c1 c2 = Ord.lift num_of_var Int.compare c1 c2

let cmp_num_symb c1 c2 = Ord.lift num_of_symb Int.compare c1 c2

let cmp_num_lits c1 c2 = Ord.lift length Int.compare c1 c2

let cmp_age c1 c2 =
  let when_born1 = get_ps_when_born c1 in
  let when_born2 = get_ps_when_born c2 in
  Int.compare (when_born2) (when_born1)

let cmp_ground c1 c2 = Ord.lift is_ground Bool.compare c1 c2

let cmp_horn c1 c2 = Ord.lift is_horn Bool.compare c1 c2

let cmp_epr c1 c2 = Ord.lift is_epr Bool.compare c1 c2

(*
let cmp_in_unsat_core c1 c2 = Ord.lift in_unsat_core Bool.compare c1 c2
*)
let cmp_has_eq_lit c1 c2 = Ord.lift has_eq_lit Bool.compare c1 c2

let cmp_has_conj_symb c1 c2 = Ord.lift has_conj_symb Bool.compare c1 c2

let cmp_has_bound_constant c1 c2 = Ord.lift has_bound_constant Bool.compare c1 c2

let cmp_has_next_state c1 c2 = Ord.lift has_next_state Bool.compare c1 c2

let cmp_has_reachable_state c1 c2 = Ord.lift has_reachable_state Bool.compare c1 c2

let cmp_has_non_prolific_conj_symb c1 c2 = Ord.lift has_non_prolific_conj_symb Bool.compare c1 c2

let cmp_bc_imp_inh c1 c2 = Int.compare (get_bc_imp_inh c2) (get_bc_imp_inh c1) 
...
let cmp_max_atom_input_occur c1 c2 =
  ...
  match d1, d2 with
  | Def(i1), Def(i2) -> Symbol.compare i1 i2  (* andrepd: was polymorphic compare of symbols here *)
	(* Undef is greater then Def *)
  | Undef, Def _ -> Ord.lt
  | Def _, Undef -> Ord.gt
  | Undef, Undef -> Ord.eq
...
(**
   Compares two clauses wrt their concreteness, concrete clauses are bigger. In
   Abstraction-Refinement, it is used for priority queues.
*)
let cmp_abstr_ref_concr c1 c2 =
  let arc1 = get_ps_abstr_ref_concr c1 in
  let arc2 = get_ps_abstr_ref_concr c2 in
  Bool.compare arc1 arc2

(* we need to uniquely generate rnd numebr for each clause to be consistent e.g. in queues *)
let cmp_rnd c1 c2 =
  Lib.compare c1.cl_rnd c2.cl_rnd 
    
let cl_cmp_type_to_fun ?(score_cmp=cmp_const_eq) ?(external_score_cmp=cmp_const_eq)  t =
  let open Options in 
  match t with
  | Cl_Age b -> compose_sign b cmp_age
  | Cl_Num_of_Var b -> compose_sign b cmp_num_var
  | Cl_Num_of_Symb b -> compose_sign b cmp_num_symb
  | Cl_Num_of_Lits b -> compose_sign b cmp_num_lits
  | Cl_Score b -> compose_sign b score_cmp
  | Cl_External_Score b -> compose_sign b external_score_cmp
  | Cl_Ground b -> compose_sign b cmp_ground
  | Cl_Conj_Dist b -> compose_sign b cmp_conj_dist
  | Cl_Has_Conj_Symb b -> compose_sign b cmp_has_conj_symb
  | Cl_has_bound_constant b -> compose_sign b cmp_has_bound_constant
  | Cl_has_next_state b -> compose_sign b cmp_has_next_state
  | Cl_has_reachable_state b -> compose_sign b cmp_has_reachable_state
  | Cl_Has_Non_Prolific_Conj_Symb b -> compose_sign b cmp_has_non_prolific_conj_symb
  | Cl_Max_Atom_Input_Occur b -> compose_sign b cmp_max_atom_input_occur
  | Cl_Horn b -> compose_sign b cmp_horn
  | Cl_EPR b -> compose_sign b cmp_epr
 (* | Cl_in_unsat_core b -> compose_sign b cmp_in_unsat_core *)
  | Cl_Has_Eq_Lit b -> compose_sign b cmp_has_eq_lit
  | Cl_min_defined_symb b -> compose_sign b cmp_min_defined_symb
  | Cl_bc_imp_inh b -> compose_sign b cmp_bc_imp_inh
  | Cl_abstr_ref_concr b -> compose_sign b cmp_abstr_ref_concr
  | Cl_rnd b ->  compose_sign b cmp_rnd
  | _ -> failwith ("clause compare option is currenlty not supported: "^(cl_cmp_type_to_str t)^"\n")

let cl_cmp_type_list_to_lex_fun ?(score_cmp=cmp_const_eq) ?(external_score_cmp=cmp_const_eq) l =
  lex_combination2 
    (lex_combination (List.map (cl_cmp_type_to_fun ~score_cmp ~external_score_cmp) l))
    (Ord.reverse_f compare)
...
let cl_measure_to_fun cl_measure = 
  ...
  |CM_num_lit  -> length
  |CM_num_var  -> num_of_var
  |CM_num_symb -> num_of_symb
  |CM_cnt      -> (fun _ -> 1)
  |CM_none     -> (fun _ -> 0)


(*---------- Proof dot out options ------------------*)

let is_unit c = 
  match (get_lits c) with 
  | [] | [_] -> true (* empty clause also considered to be unit *)
  | _-> false 

let is_all_neg c = 
  not (List.exists Term.is_pos_lit (get_lits c))
    
let is_all_pos c = 
  not (List.exists Term.is_neg_lit (get_lits c))

    
let proof_reduce_dot_to_fun c prd_opt = 
  let open Options in
  match prd_opt with 
  |PRD_unit -> is_unit c
  |PRD_all_neg -> is_all_neg c
  |PRD_all_pos -> is_all_pos c
  |PRD_ground -> is_ground c
        
(* true if at least one condition is true *)      
let proof_reduce_dot_list_to_filter prd_opt_list c = 
  List.exists (proof_reduce_dot_to_fun c) prd_opt_list
...
(* Clause is generated by a global propositional subsumption *)
let tstp_source_global_subsumption max_clause_id parent =
  (TSTP_inference_record (Global_subsumption max_clause_id, [parent]))

(* Clause is generated by a global propositional subsumption; justified *)
let tstp_source_global_subsumption_just parents =
  (TSTP_inference_record (Global_subsumption_just, parents))

(* Clause is generated by a global propositional implication *)
let tstp_source_prop_impl max_clause_id =
  (TSTP_inference_record (Prop_impl max_clause_id, []))

(* Clause is generated by a global propositional implication; justified *)
let tstp_source_prop_impl_just parents =
  (TSTP_inference_record (Prop_impl_just, parents))

(* Clause is generated by a global smt implication; justified *)
let tstp_source_smt_impl_just parents =
  (TSTP_inference_record (SMT_impl_just, parents))
...
let tstp_source_well_definedness parent =
  (TSTP_inference_record (WellDefinedness, [parent]))
...
let tstp_source_bin_hyper_res main_parent parents =
     (TSTP_inference_record
        (Bin_hyper_res, (main_parent :: parents)))

(* Clause is generated in a backward subsumption resolution *)
let tstp_source_backward_subsumption_resolution main_parent parents =
  (TSTP_inference_record (Backward_subsumption_resolution, main_parent :: parents))
...
let tstp_source_sig_abstr parent =
  (TSTP_inference_record (Sig_abstr, [parent]))

let tstp_source_subs_abstr parent =
  (TSTP_inference_record (Subs_abstr, [parent]))

let tstp_source_gen_abstr parent =
  (TSTP_inference_record (Gen_abstr, [parent]))
...
let tstp_source_superposition ~left ~right = 
  TSTP_inference_record (Superposition, [left; right])

let tstp_source_equality_resolution clause = 
  TSTP_inference_record (Equality_resolution, [clause])

let tstp_source_equality_factoring clause = 
  TSTP_inference_record (Equality_factoring, [clause])

let tstp_source_demodulation ~main ~eqs = 
  TSTP_inference_record (Demodulation, main :: eqs)

let tstp_source_lightnorm ~main ~eqs = 
  TSTP_inference_record (LightNormalisation, main :: eqs)

let tstp_source_theory_normalisation ~main ~axioms = 
  TSTP_inference_record (TheoryNormalisation, main :: axioms)

let tstp_source_smt_theory_normalisation main = 
  TSTP_inference_record (SMTTheoryNormalisation, [main])

let tstp_source_ac_demodulation ~main ~eqs ~axioms = 
  TSTP_inference_record (ACDemodulation, main :: (eqs @ axioms))

let tstp_source_ground_joinability ~main ~eqs = 
  TSTP_inference_record (GroundJoinability, main :: eqs)

let tstp_source_connectedness ~main ~eqs = 
  TSTP_inference_record (Connectedness, main :: eqs)
...
let pp_tstp_theory ppf = function
  ...
  | TSTP_arith    -> Format.fprintf ppf "arith"
...
let pp_inference_rule parents ppf = function
  ...
  | Superposition -> 
      Format.fprintf
  ppf
  "superposition,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  | Equality_factoring -> 
      Format.fprintf
  ppf
  "equality_factoring,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  | Equality_resolution -> 
      Format.fprintf
  ppf
  "equality_resolution,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  | Predicate_to_equality -> 
      Format.fprintf
  ppf
  "predicate_to_equality,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  | Predicate_to_equality_rev -> 
      Format.fprintf
  ppf
  "predicate_to_equality_rev,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  | WellDefinedness -> 
      Format.fprintf
  ppf
  "well_definedness,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  ...
  | Demodulation -> 
      Format.fprintf
	ppf
	"demodulation,@,[status(thm)],@,@[<hov 1>[%a]@]"
	(pp_any_list pp_clause_name ",")
	parents
  | LightNormalisation -> 
      Format.fprintf
  ppf
  "light_normalisation,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  ...
  | ACDemodulation -> 
      Format.fprintf
  ppf
  "ac_demodulation,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  ...
  | GroundJoinability -> 
      Format.fprintf
  ppf
  "ground_joinability,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  ...
  | Connectedness -> 
      Format.fprintf
  ppf
  "connectedness,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  ...
  | TheoryNormalisation -> 
      Format.fprintf
  ppf
  "theory_normalisation,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  ...
  | SMTTheoryNormalisation -> 
      Format.fprintf
  ppf
  "smt_theory_normalisation,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  ...
  | SMT_implied -> 
      Format.fprintf
  ppf
  "smt_implied,@,[status(thm)],@,@[<hov 1>[%a]@]"
  (pp_any_list pp_clause_name ",")
  parents
  ...
  | Global_subsumption _ ->
      ...
      Format.fprintf
	ppf
	"global_subsumption,@,[status(thm)],@,@[<hov 1>[%a]@]"
	(pp_any_list pp_clause_name ",")
	parents
  ...
  | Global_subsumption_just ->
      ...
      Format.fprintf
	ppf
	"global_subsumption_just,@,[status(thm)],@,@[<hov 1>[%a]@]"
	(pp_any_list pp_clause_name ",")
	parents
  ...
  | Prop_impl_just ->
      ...
      Format.fprintf
	ppf
	"prop_impl_just,@,[status(thm)],@,@[<hov 1>[%a]@]"
	(pp_any_list pp_clause_name ",")
	parents
  ...
  | SMT_impl_just ->
      ...
      Format.fprintf
	ppf
	"smt_impl_just,@,[status(thm)],@,@[<hov 1>[%a]@]"
	(pp_any_list pp_clause_name ",")
	parents
  ...
  | Bin_hyper_res ->
      Format.fprintf
      ppf
      "bin_hyper_res,@,[status(thm)],@,@[<hov 1>[%a]@]"
      (pp_any_list pp_clause_name ",")
      parents
  ...
  | Arg_filter_abstr ->
    Format.fprintf
      ppf
      "arg_filter_abstr,@,[status(unp)],@,@[<hov 1>[%a]@]"
      (pp_any_list pp_clause_name ",")
      parents
 ...
 *)
let pp_tstp_inference_record prop_impl_justification_fun clause ppf = function
    
    (* Must get justification for global propositional subsumption here *)
  | Global_subsumption max_clause_id, [parent] ->
      
      (match prop_impl_justification_fun with
      | Some (gs_just_fun) ->
          begin
	    try 
	      ...
	      Format.fprintf
            ...
            with
	    Failure _ -> 
              (out_warning ("clause.ml:global_propositional_subsumption: failed to get parents for"^(to_string clause));
               Format.fprintf
	         ppf
  ...
  | Prop_impl max_clause_id, _ ->       
	  ...
	  (* Get justification for simplification of clause *)
	    ...
	    Format.fprintf
          ...
          with
	    Failure _ -> 
              (out_warning ("clause.ml:prop_impl:failed to get parents for"^(to_string clause));
               Format.fprintf
	         ppf
	         "prop_impl,@,[status(thm)],@,@[]"
              )
          end
      | None ->
	  (* output without global subsumtion justification *)
...
let pp_tstp_source prop_impl_justification_fun clausify_proof clause ppf = function
  ...
  | TSTP_inference_record inference_record ->
	...
	ppf
	"@[<hv 10>inference(%a)@]"
	(pp_tstp_inference_record prop_impl_justification_fun clause)
	inference_record

(* Print clause with source in TSTP format *)
let pp_clause_with_source ?(prop_impl_justification_fun = None) ?(clausify_proof = false)  ppf clause =
  let c_type =   
    (if (is_negated_conjecture clause)
    then "negated_conjecture"
    else "plain")
  ...
  Format.fprintf
    ...
    (pp_tstp_source prop_impl_justification_fun clausify_proof clause)
    (get_tstp_source clause)

let pp_clause_list_with_source ?(prop_impl_justification_fun = None) ?(clausify_proof = false)  ppf clause_list =
  List.iter (pp_clause_with_source ppf ~prop_impl_justification_fun ~clausify_proof) clause_list
...
let pp_clause_params param_out_list ppf c =
  let param_name p = (Pair.first p) in
  let param_fun p = (Pair.second p) in
  let max_param_length =
    let max_pair =
      list_find_max_element
        (fun p1 p2 -> Int.compare (String.length (param_name p1)) (String.length (param_name p2)))
        param_out_list
    in
...
(*---------------------------------*)
let rec get_demod_main clause = 
  match get_tstp_source clause with
  | TSTP_inference_record (Demodulation,       main :: _) 
  | TSTP_inference_record (LightNormalisation, main :: _) ->
  (* Needs ACDemodulation here? *)
    main
  | TSTP_inference_record (Eq_res_simp, [c]) ->
    get_demod_main c
  | _ -> failwith (sprintf "Clause.get_demod_main: not a demod/lightnorm conclusion %s" (to_string_tptp clause))

let rec get_demod_eqs clause = 
  match get_tstp_source clause with
  | TSTP_inference_record (Demodulation,       _ :: eqs) 
  | TSTP_inference_record (LightNormalisation, _ :: eqs) ->
  (* Needs ACDemodulation here? *)
    eqs
  | TSTP_inference_record (Eq_res_simp, [c]) ->
    get_demod_eqs c
  | _ -> failwith (sprintf "Clause.get_demod_eqs: not a demod/lightnorm conclusion %s" (to_string_tptp clause))

let rec get_simp_parents' acc clause clause' = 
  if Bc.(clause == clause') then
    acc
  else
    match get_tstp_source clause' with
    | TSTP_inference_record (Global_subsumption _, main :: ([] as side))
    | TSTP_inference_record (Global_subsumption_just, main::side) 
    | TSTP_inference_record (Demodulation, main :: side)
    | TSTP_inference_record (LightNormalisation, main :: side)
    | TSTP_inference_record (Forward_subsumption_resolution, main :: side)
    | TSTP_inference_record (Backward_subsumption_resolution, main :: side) (* KK ask why this was commented! *)
    | TSTP_inference_record (Eq_res_simp, main :: ([] as side))
    | TSTP_inference_record (Dis_eq_elim, main :: ([] as side))
    | TSTP_inference_record (TheoryNormalisation, main :: side)
    | TSTP_inference_record (SMTTheoryNormalisation, main :: ([] as side))
    | TSTP_inference_record (ACDemodulation, main :: side)
    | TSTP_inference_record (GroundJoinability, main :: side)
    | TSTP_inference_record (Connectedness, main :: side)
    | TSTP_inference_record (Renaming, main :: ([] as side))
    | TSTP_inference_record (Unflattening, main :: ([] as side))
    ->
      get_simp_parents' (side @ acc) clause main
    | _ -> 
        (Format.printf "Clause.get_simp_parents': unrecognised TSTP_inference_record: %a@.\n" (pp_clause_with_source ~prop_impl_justification_fun:None ~clausify_proof:true) clause');
      failwith ("Clause.get_simp_parents': unrecognised TSTP_inference_record:")


let get_simp_parents clause clause' = 
  get_simp_parents' [] clause clause'
  ...
  then
   ...
   (* let found_c =  (context_find cc c) in *)
     ...
     *)
     ...
     in 
     Statistics.(bump_int_stat simp_replaced_by);
(*			
...
module Clause_Dism = struct 
  type t = {
      clause : clause;
      mutable dism   : Dismatching.constr_set;
    }
        
  let create c =
    {
     clause = c;
     dism =  Dismatching.create_constr_set ();
   }

  let add_to_dism_constr subst_norm c_d = 
    c_d.dism <- Dismatching.add_constr c_d.dism subst_norm

(*  let is_not_redundant_inst_norm ~dismatching_flag ~add_to_dism subst_norm clause_param = *)
(* true if sat; add subst to constr. if sat *)
  let check_and_add subst_norm c_d = 
    let res =
      try 
        c_d.dism <- Dismatching.check_and_add c_d.dism subst_norm;
        true
      with Dismatching.Constr_Not_Sat ->
        Statistics.(bump_int_stat inst_num_of_dismatching_blockings);
        false
    in
    res
(*KK: for some reason was true in place res*)
(*    true *)


  let dism_to_string dism =   
(*    try *)
      "["^(Dismatching.to_string_constr_set dism)^"]"
(*    with
      Dismatching_undef -> "[]"
  *)        
  let to_string c_d = 
    (to_string c_d.clause)^" "^(dism_to_string c_d.dism)^"\n"
end
