...
(* clauses for exchange e.g. with instantiation *)
let exchange_clauses = ref []
let add_active_to_exchange_flag  = ref false
let add_passive_to_exchange_flag = ref false

let add_active_to_exchange clause = 
  if !add_active_to_exchange_flag
  then exchange_clauses := clause::!exchange_clauses
  else ()

let add_passive_to_exchange clause = 
  if !add_passive_to_exchange_flag
  then exchange_clauses := clause::!exchange_clauses
  else ()
...
(*************add new clauses*******)    
let add_new_clause_to_passive given_clause clause = 
...
(*	passive_queue_ref := 
	...
	then raise Given_clause_is_dead 
...
let rec discount_loop_body () =
	...
	(try
	  ...
	  let given_clause = simplify_forward_heavy_old feature_list clause in
	(* Clause.set_bool_param false Clause.in_passive given_clause; *)	
          simplify_backward_heavy feature_list given_clause; 
	  let selected_literals = selection_fun given_clause in
	  (*out_str_debug ("given_clause: "^(Clause.to_string  given_clause)
			 ^"selected lit: "
			 ^(Term.term_list_to_string selected_literals));  *)
          
	  all_factorings   given_clause selected_literals;          
...
let init_discount_init_clauses clause_list = 
  let renew_clauses_in_init clause = 
    Clause.create (Clause.get_literals clause) in
  init_clause_list_ref := (List.map renew_clauses_in_init clause_list);
  init_discount ()
...
let discount_loop_exchange act_to_exch_flag pass_to_exch_flag = 
  let old_active_flag = !add_active_to_exchange_flag in
  let old_passive_flag = !add_passive_to_exchange_flag in 
  add_active_to_exchange_flag := act_to_exch_flag;
  add_passive_to_exchange_flag := pass_to_exch_flag;
  exchange_clauses := [];
  discount_loop_body ();
(*  out_str ("Discount Exch Clauses: "
	   ^(Clause.clause_list_to_string !exchange_clauses)^"\n");*)
  (*let new_exchange_clauses =  exchange_clauses in
    exchange_clauses := [];*)
  add_active_to_exchange_flag  := old_active_flag;
  add_passive_to_exchange_flag := old_passive_flag;
(*  !new_exchange_clauses*)
  !exchange_clauses
