...
module SIndexM = Trie_func.Make(Term.TKey)

type index = {
  mutable ind: clause SIndexM.trie
} (* [@@unboxed] *)
...
let rec is_subsumed' index lit_list =
  try 
    SIndexM.long_or_in lit_list index.ind
  with Not_found -> 
    begin match lit_list with 
    | l::tl -> is_subsumed' index tl
    | [] -> raise Not_found
    end
...
let is_subsumed index clause = 
  is_subsumed' index (Clause.get_literals clause)
